...
class Validator extends React.Component {
  ...
  handleStakeSubmitClick = async event => {
    ...
    const tx = await contract.stakeDAI(ethers.utils.bigNumberify(daiAmount));
    tx.wait();
  };
  ...
  handleSetWCCAddress = async () => {
    ...
    console.log("TCL: Validator -> handleSetWCCAddress -> contract", contract);
    console.log(`new wcc => ${wccAddress}`);
    try {
      const tx = await contract.setWCCContract(wccAddress);
      tx.wait();
      console.log(tx);
    } catch (e) {
      console.error(e);
  ...
  render() {
    ...
    return (
      <Flex flexDirection={"column"} alignItems={"center"}>
        ...
        <Card width={1 / 2} alignItems={"center"}>
